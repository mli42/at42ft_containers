...
/*   Updated: 2021/03/07 16:09:16 by mli              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
map<Key, T, Compare, Alloc>::map(
    typename ft::enable_if<!std::numeric_limits<Ite>::is_integer, Ite>::type \
    first, Ite last, const key_compare &comp, const allocator_type &alloc) : \
...
void    map<Key, T, Compare, Alloc>::_btree_rm(node_ptr rmNode) {
    ...
    --this->_size;
    if (rmNode->parent)
        rmPlace = (rmNode->parent->left == rmNode ? \
            &rmNode->parent->left : &rmNode->parent->right);
...
template<class Key, class T, class Compare, class Alloc>
bool
map<Key, T, Compare, Alloc>::_key_eq(const key_type &k1,
